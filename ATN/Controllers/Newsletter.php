...
class ATN_Controllers_Newsletter extends ATN_Base{
    ...
    public function newsletter_article(){
        ...
        if(  isset($form_post['wp']) ){
            ...
            $query = atn_get_posts(
                $wp_forms['posts_per_page'], 
                $wp_forms['last_date_query'],
                isset($wp_forms['category']) ? $wp_forms['category'] : array()
            );
        ...
        if( isset($form_post['events'])
            ...
            if( isset($event_forms['event-category']) ) {
                $event_category = $event_forms['event-category'];
            }
            
            $event_get_query = atn_get_events($event_forms['numberposts'], $event_forms['event_start_before'], $event_category);
            if( $event_get_query ){
        ...
        $data['gall_post_per_page'] = isset($post_input['gallery']['posts_per_page']) ? $post_input['gallery']['posts_per_page']:'';
        $data['gall_show_upcoming_days'] = isset($post_input['gallery']['last_date_query']) ? $post_input['gallery']['last_date_query']:'';
        //print_r($form_post);
